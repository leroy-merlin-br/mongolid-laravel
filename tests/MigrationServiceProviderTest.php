...
class MigrationServiceProviderTest extends TestCase
{
    public function testShouldBoot()
    {
        // Set
        $provider = m::mock(
            MigrationServiceProvider::class . '[commands]',
            [$this->app]
        );
    ...
    public function testShouldRegister()
    {
        ...
        $this->assertInstanceOf(
            MongolidMigrationRepository::class,
            $this->app['mongolid.migration.repository']
        );
        $this->assertInstanceOf(
            Migrator::class,
            $this->app['mongolid.migrator']
        );
        $this->assertInstanceOf(
            MigrationCreator::class,
            $this->app['mongolid.migration.creator']
        );
        $this->assertInstanceOf(
            MigrateCommand::class,
            $this->app['command.mongolid-migrate']
        );
        $this->assertInstanceOf(
            FreshCommand::class,
            $this->app['command.mongolid-migrate.fresh']
        );
        $this->assertInstanceOf(
            InstallCommand::class,
            $this->app['command.mongolid-migrate.install']
        );
        $this->assertInstanceOf(
            MigrateMakeCommand::class,
            $this->app['command.mongolid-migrate.make']
        );
        $this->assertInstanceOf(
            RefreshCommand::class,
            $this->app['command.mongolid-migrate.refresh']
        );
        $this->assertInstanceOf(
            ResetCommand::class,
            $this->app['command.mongolid-migrate.reset']
        );
        $this->assertInstanceOf(
            RollbackCommand::class,
            $this->app['command.mongolid-migrate.rollback']
        );
        $this->assertInstanceOf(
            StatusCommand::class,
            $this->app['command.mongolid-migrate.status']
        );
    }
